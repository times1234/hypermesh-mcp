...
| `target_surf` | 偏移轴线识别时，与源面成对的另一端截面 |
| `axis_mode` | `global` 表示使用 X/Y/Z 方向，`oblique` 表示使用探针识别出的偏移轴线 |
| `axis_vec` | 偏移轴线方向向量，供 drag 和 spin 共同使用 |
| `axis_p0` / `axis_p1` | 偏移轴线两端的参考点 |
| `drag_distance_hint` | 偏移源面到目标面的距离，优先作为 drag 距离 |
| `gear_axis` | 推测的齿轮轴向 |
...
如果按全局 X/Y/Z 找不到可靠源面，且实体 surface 数量不高，探针会临时细分各个 surface，寻找一对面积相近、法向近似相反、中心连线与法向一致的端面。斜轴候选还必须让源面到目标面的距离接近实体最薄方向尺寸，避免把两片侧面误当成端面。找到后会输出 `axis_mode=oblique`、`axis_vec`、`axis_p0/p1` 和 `drag_distance_hint`。这类零件本身仍是恒截面件，只是轴线相对全局坐标有倾角，所以后续 drag/spin 不再强行使用 X/Y/Z，而是用这条真实偏移轴线。
...
- 小型密边界圆环误识别保护：对 `surf_count` 约 `120..179`、厚径比约 `0.20..0.32`、单内环且源面边界节点异常密集的圆形件，会再看 `src_boundary_nodes / surf_count` 和 `gear_tooth_count / surf_count`。如果边界/面数比高到约 `1.45` 以上，但齿面密度低于约 `0.70`，则认为更像轴承/凸轮类密边界圆环，不进入齿轮路径。
- 高面数外齿环：没有稳定 `src_surf`，但 `surf_count` 很高，外圈齿面数量很多，例如大外齿环。代码会在 `surf_count >= 400`、圆形包围盒比例合理、且探针确实找到了足够重复齿面的前提下归入齿轮；齿面候选主要看中心半径、外半径、局部跨度和角度跨度，不再把 bbox 内半径作为普遍硬条件，避免真实外齿因为斜面 bbox 误差被清空。对 s2/s36 这类 `19 x 75 x 75` 薄盘，名义外半径路径允许齿面轴向占比低到约 `0.06`，以覆盖真实齿面 `axis_ratio ~= 0.075` 的情况；但名义外圈 tip/flank 仍要求中心半径按对角半径归一化后不低于约 `0.62`，且 flank 的 bbox 内半径不低于名义外半径的约 `0.62`，用于排除 s3 这类大外齿盘里伸向内圈的宽壁面；另有一条窄轴向齿侧面条件覆盖 `axis_ratio ~= 0.02`、局部跨度很小、外半径接近名义外径的齿面。仍用外圈半径、局部跨度和角度跨度排除内侧孔壁、辐条边界、宽环面和普通盘面。
...
`surf_count == 6` 的简单恒截面实体或单内环短圆环优先走 drag。这里不要求厚径比特别薄，只要 `mn / mx` 不超过约 `0.75` 且源面明确即可；这样 s9 这类短厚圆筒也会走 drag，同时近似立方体的普通实体仍不会被强行归入 drag。
...
对偏移轴线的恒截面件，目前只允许最简单的 `surf_count == 4/6` 进入 vector drag。回转件进入 spin 时，如果探针已经找到可靠的斜向端面对，且端面距离通过最薄尺寸校验，则 spin 使用这条 `axis_vec` 作为真实旋转轴；否则仍优先使用全局 bbox 推断出来的轴向，避免把不可靠的侧面对向量当成旋转轴。
...
| 小太阳轮/小外齿轮 | 紧凑圆形实体，源面边界密集，齿数较多 | 限定 `surf_count`、厚径比、内环数量和边界节点数；对 s29 这类单内环但源面边界节点相对面数异常密、齿面密度不足的圆形件，先按非齿轮排除 |
| 高面数外齿环 | 无稳定源面，`surf_count >= 400`，圆形外包络明显，探针找到重复齿面；名义外半径路径覆盖 `axis_ratio` 约 `0.06` 以上的齿面，并用窄轴向齿侧面条件补齐 `axis_ratio` 约 `0.02` 的齿面 | 候选面中心半径和外半径要靠近外圈；名义外圈 tip/flank 要求中心半径不进入内圈带，flank 还要求 bbox 内半径不进入内圈带，避免 s3 内侧宽壁面误判；同时限制局部跨度、角度跨度和径向跨度，避免复杂圆壳、内侧孔壁、宽环面或辐条边界误判 |
| 轴上局部齿圈 | 实体沿轴向较长，横截面接近圆，齿圈可能位于轴中间 | 必须探测到重复齿面；普通圆轴因为 `gear_tooth_count=0` 不会进齿轮 |
| 厚齿轮/倒角齿轮 | 外形较厚或带倒角，齿面候选密度低于薄盘齿轮 | 需要紧凑包围盒和足够重复齿面；对无源面的高面数近圆倒角体，重复面密度过高时不走该兜底规则，避免 s30 这类非齿轮被倒角阵列误判 |
...
drag 或 spin 失败的实体会回退到 tetra。tetra 队列中齿轮会优先成批执行；高风险实体仍会单独成批以降低 HyperMesh 崩溃风险。cut-section spin 如果已经把实体切成多个 solid 后失败，这些 split solid 会作为同一个 tetra 分组进入同一批次，避免原 solid 和新生成的大 ID solid 被普通 ID 排序拆到不同时间划分。
...
对很小的真实切截面，`section_minor <= 3` 且 `section_major <= 6` 时，最少 shell 数上限会放宽到 `4`。对已经通过切分平面筛选的真实截面，`min_shells` 上限限制为 `30`；如果节点到切平面的距离合格且 quad 数不少于 `6`，也允许作为 spin 种子截面。这样避免窄折线截面因为 bbox 面积估算过大而被误判为 shell 数不足。仍然要求截面 shell 节点贴在切平面容差内，避免把侧壁或端面误当成旋转截面；最终用于 spin 的截面还必须至少有 `4` 个 shell 且至少 `4` 个 quad，防止只有 2 个 quad 的倒角/窄缝小面被误判为成功截面。

旋转份数按半径自适应：

```text
raw_density = ceil((2 * pi * max_radius / section_size) * 0.85)
spin_density = clamp(raw_density, spin_density_min, spin_density_max)
```

其中 `max_radius` 来自截面 shell 节点到旋转轴的最大距离。

如果 Phase 1 已经识别出 `axis_mode=oblique`，spin 的旋转轴可以是 `spin_axis=vector`。此时半径计算使用节点到任意空间轴线的距离，而不是只按 X/Y/Z 坐标差计算；切分平面仍要求包含这条真实旋转轴。
...
1. 通过旋转轴切一个横截面。
2. 切分后会得到新的截面 surface。代码先记录切分前的 surface 集合，切分后优先只允许新增 surface 进入候选；候选还要通过切分平面 bbox 距离筛选。若同一切平面上有多份候选，不再按 surface ID 取第一个，而是按候选截面的 bbox 面积和主跨度选择最大的一份作为 spin 种子截面，避免把实体原有的倒角、窄缝或局部小面当成截面。
3. 在筛选出的真实截面上划分 2D 网格。
4. 将截面网格绕轴旋转生成 3D 六面体。
5. 如果失败，回退到 tetra。

当前唯一支持的 spin 路径就是 cut-section spin：切出新截面、按切分平面筛选真实截面、在真实截面上划 2D、再 spin 成体。代码不再使用旧的直接源面 spin 路径，也不会把已有预切分表面当作截面兜底。
...
- 先根据最大旋转半径估算周长。
- 用周长除以截面目标尺寸，再乘以 `0.85` 得到建议份数。
- 再夹在 `spin_density_min` 和 `spin_density_max` 之间。
- 默认旋转份数下限为 `60`，上限为 `160`；离线弹窗中可以设置旋转份数上下限。

如果 spin 失败或生成结果不满足要求，代码会把该 solid 重新纳入 tetra 队列。为了避免之前出现的“solid 被切成两半后只划一半”的问题，失败回退时会把原 solid 和切分产生的新 solid 标记为同一个 tetra 分组，并在后续 tetra 分批时保持同批执行。
