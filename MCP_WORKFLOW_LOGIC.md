...
对很小的真实切截面，`section_minor <= 3` 且 `section_major <= 6` 时，最少 shell 数上限会放宽到 `4`。对已经通过切分平面筛选的真实截面，`min_shells` 上限限制为 `30`；如果节点到切平面的距离合格且 quad 数不少于 `6`，也允许作为 spin 种子截面。这样避免窄折线截面因为 bbox 面积估算过大而被误判为 shell 数不足。仍然要求截面 shell 节点贴在切平面容差内，避免把侧壁或端面误当成旋转截面；这个保护用于 s27/s28 这类切面很小但确实可 spin 的环形件。
...
1. 通过旋转轴切一个横截面。
2. 切分后会得到两个截面，代码会扫描切分后 solid 上的 surface，用切分平面做 bbox 距离筛选，只保留真正贴近切平面的截面候选，而不是直接把所有新增 surface 都当成截面；如果同一切平面上得到两份重复截面，只保留一个作为 spin 种子截面。
